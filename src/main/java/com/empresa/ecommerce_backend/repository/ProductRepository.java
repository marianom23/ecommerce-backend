...
public interface ProductRepository extends BaseRepository<Product, Long>, JpaSpecificationExecutor<Product> {
      ...
      where exists (
          ...
          where v.product = p
            and (
                 ...
                 or v.fulfillmentType = com.empresa.ecommerce_backend.enums.FulfillmentType.DIGITAL_ON_DEMAND
            )
      )
    """,
            countQuery = """
    ...
    where (:namePattern is null or lower(p.name) like :namePattern)
      and ( :inStockOnly is null or :inStockOnly = false
            or ( (v.stock is not null and v.stock > 0)
                 or v.fulfillmentType = com.empresa.ecommerce_backend.enums.FulfillmentType.DIGITAL_ON_DEMAND ) )
